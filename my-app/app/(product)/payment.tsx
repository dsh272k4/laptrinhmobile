...
export default function PaymentScreen() {
  ...
  const fetchQRCode = async (orderRequest: { totalAmount: number; }) => {
    try {
      const response = await axios.post("http://192.168.11.23:8080/api/orders/payment", orderRequest);
  ...
  const handleCreateQRCode = async () => {
    ...
    console.log("Mã QR đã được tạo thành công:", data.qrCodeUrl);
  };
  useEffect(() => {
    handleCreateQRCode();
  }, []);
  ...
  const updateProductStock = (productId: number, quantity: number) => {
    return axios.put(`http://192.168.11.23:8080/api/product/${productId}/update-stock`, {
      quantity: quantity,
    })
      .catch((error) => {
  ...
  const handlePaymentSuccess = () => {
    ...
    Promise.all(updateStockPromises)
      .then(() => {
        ...
        return axios.post('http://192.168.11.23:8080/api/orders/create', orderRequest, {
          timeout: 20000,
        });
      })
      .then(() => {
        ...
        return axios.delete(`http://192.168.11.23:8080/api/cart/delete/${userInfo.id}`);
      })
      .then(() => {
        ...
        return axios.post('http://192.168.11.23:8080/api/email-order/success', emailRequest, {
          timeout: 20000,
        });
      })
      .then(() => {
  ...
  const handlePaymentCashSuccess = () => {
    ...
    Promise.all(updateStockPromises)
      .then(() => {
        ...
        axios.post('http://192.168.11.23:8080/api/orders/create-cash', orderRequest,
          { timeout: 10000 })
          .then(() => {
            if (!userInfo) {
            ...
            // Sau khi tạo đơn hàng thành công, xóa giỏ hàng
            axios.delete(`http://192.168.11.23:8080/api/cart/delete/${userInfo.id}`)
              .then(() => {
                Toast.show({
                  type: "success",
