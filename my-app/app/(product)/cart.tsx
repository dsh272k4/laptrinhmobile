...
export default function CartScreen() {
  ...
  const increaseQuantity = (id: number) => {
    setProducts((prevProducts) =>
      prevProducts.map((product) => {
        if (product.id === id) {
          // Kiểm tra nếu số lượng hiện tại đã đạt giới hạn trong kho
          ...
          // Gửi yêu cầu cập nhật số lượng sản phẩm trên backend
  ...
  const removeProduct = (productId: number) => {
    if (userInfo) {
      axios.delete(`http://192.168.11.23:8080/api/cart/remove/${userInfo.id}/${productId}`)
        .then(() => {
          setProducts((prevProducts) =>
    ...
    try {
      if (userInfo) {
        if (newQuantity > 0) {  // Kiểm tra số lượng phải lớn hơn 0 trước khi gửi yêu cầu
          await axios.put(`http://192.168.11.23:8080/api/cart/update/${userInfo.id}/${id}`, { quantity: newQuantity });
        }
      }
    } catch (error) {
