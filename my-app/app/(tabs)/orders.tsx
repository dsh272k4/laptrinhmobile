...
export default function OrderScreen() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await axios.get(`http://192.168.11.23:8080/api/orders/user/${userInfo.id}`);
  ...
  const handleDeleteOrder = async (orderId: number, orderStatus: string) => {
    ...
    try {
      const response = await axios.delete(`http://192.168.11.23:8080/api/orders/delete/${orderId}`);
      Toast.show({
        type: 'success',
        text1: 'Thành công',
  ...
  const handleShowDetails = async (orderId: number) => {
    try {
      const response = await axios.get(`http://192.168.11.23:8080/api/orders/${orderId}`);
  ...
  const renderOrderItem = ({ item }: { item: Order }) => (
    <TouchableOpacity style={styles.orderCard}>
      <View style={styles.cardContent}>
          ...
          <View style={styles.statusContainer}>
            {statuses
              .filter((status) => {
                ...
                if (item.status === 'DELIVERED_SUCCESSFULLY' && status.key === 'DELIVERY_FAILED') {
                  return false;
                }
                // Loại bỏ "Giao hàng thành công" nếu trạng thái là "Giao hàng thất bại"
                if (item.status === 'DELIVERY_FAILED' && status.key === 'DELIVERED_SUCCESSFULLY') {
                  return false;
                }
                return true;
              })
              .map((status) => (
                <View key={status.key} style={styles.statusItem}>
                  <Text style={[
                    styles.checkmark,
                    isCompleted(item.status, status.key) && { color: '#28A745' }, // Dấu tích màu xanh nếu hoàn thành
                  ]}>
                    {isCompleted(item.status, status.key) ? '✓' : '  '} {/* Dấu tích nếu đã hoàn thành */}
                  </Text>
                  <Text
                    style={[
                      styles.statusText,
                      {
                        ...
                        fontWeight: item.status === status.key ? 'bold' : 'normal', // Chữ đậm cho trạng thái hiện tại
                      },
                    ]}
                  >
                    {status.label}
        ...
        <Text style={styles.orderDate}>Thời gian đặt hàng: {formatDate(item.orderDate)}</Text>
        <Text style={styles.orderDate}>Đơn hàng cập nhật: {formatDate(item.updatedAt)}</Text>
