...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    axios
      .get<Product[]>(`http://192.168.11.23:8080/api/products/brand/${id}`)
      .then((response) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const productsResponse = await axios.get<Product[]>(
        "http://192.168.11.23:8080/api/products"
      );
      const bannerResponse = await axios.get<ItemBanner[]>(
        "http://192.168.11.23:8080/api/banners"
      );
  ...
  const fetchCartItems = async () => {
    ...
    try {
      const response = await axios.get(`http://192.168.11.23:8080/api/cart/${userInfo.id}`);
  ...
  const handleAddToCart = async (product: Product) => {
    try {
      ...
      const response = await axios.post(
        `http://192.168.11.23:8080/api/cart/add/${userInfo.id}`,
        cartItem
      );
  ...
  useEffect(() => {
    if (selectedCategoryId !== null) {
      axios
        .get<Product[]>(
          `http://192.168.11.23:8080/api/products/category/${selectedCategoryId}`
        )
        .then((response) => {
  ...
  const renderProduct = ({ item }: { item: Product }) => {
    ...
    return (
      ...
      >
        <Image
          source={{
            uri: `http://192.168.11.23/images/${item.imagePaths[selectedColorIndex]}`,
          }}
          ...
          <View style={styles.bannerContainer}>
            ...
            >
              {dataBanner.map((banner, index) => (
                <Image
                  key={index}
                  source={{ uri: `http://192.168.11.23/images/${banner.image}` }}
                  style={styles.banner}
                  resizeMode="cover"
                />
              ))}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {dataCategory.map((category) => (
                ...
                >
                  ...
                  <Image
                    source={{
                      uri: `http://192.168.11.23/images/${category.image}`,
                    }}
