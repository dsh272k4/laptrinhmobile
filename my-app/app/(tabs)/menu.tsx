...
export default function TabTwoScreen() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const response = await axios.get<ItemCategory[]>("http://192.168.11.23:8080/api/categories");
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      if (dataCategory.length > 0) { // Kiểm tra danh mục đã có dữ liệu
        try {
          const response = await axios.get<ItemBrand[]>(`http://192.168.11.23:8080/api/brands/category/${selectedCategory}`);
          if (response.data && response.data.length > 0) {
  ...
  const renderBrandItem = ({ item }: { item: ItemBrand }) => (
    ...
    >
      ...
      <Image
        source={{
          uri: item.image
            ? `http://192.168.11.23/images/${item.image}`
