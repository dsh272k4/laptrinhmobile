...
export default function TabThreeScreen() {
    ...
    if (tempUserInfo && tempUserInfo.id) {
      try {
        const response = await fetch(
          `http://192.168.11.23:8080/api/users/${tempUserInfo.id}`,
          {
            method: "PUT",
            headers: {
        ...
        if (response.ok) {
          ...
          const fetchUpdatedUserInfo = async () => {
            const fetchResponse = await fetch(
              `http://192.168.11.23:8080/api/users/${updatedUser.id}`
            );
            if (fetchResponse.ok) {
  ...
  const handleDeleteUser = async () => {
    if (userInfo && userInfo.id) {
      try {
        const response = await fetch(
          `http://192.168.11.23:8080/api/users/${userInfo.id}`,
          {
            method: "DELETE",
          }
        );
        ...
        {expanded && (
          <View style={styles.optionsContainer}>
            <TouchableOpacity
              ...
              onPress={() => Linking.openURL('https://zalo.me/0983077673983077673')}
            >
              <Image source={require('@/assets/images/zalo.jpeg')} resizeMode='contain' style={styles.icon} />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.button}
              onPress={() => { Linking.openURL("https://m.me/100068114271008?hash=AbY87uradpLPJSQb&source=qr_link_share") }}
            >
              <Image source={require('@/assets/images/message.jpg')} resizeMode='contain' style={styles.icon} />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.button}
              onPress={() => { Linking.openURL('tel:09830776730983077673'); }}
            >
