...
const BannerManagement = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await axios.get('http://192.168.11.23:8080/api/banners');
  ...
  const uploadImage = async (imageUri: string, name: string): Promise<string | null> => {
    ...
    try {
      const response = await axios.post('http://192.168.11.23:8080/api/banners', formData, {
  ...
  const handleAddBanner = async () => {
    try {
      ...
      await axios.post('http://192.168.11.23:8080/api/banners', newBannerData);
  ...
  const handleDeleteBanner = async (id: number) => {
    try {
      await axios.delete(`http://192.168.11.23:8080/api/banners/${id}`);
      fetchBanners();
    } catch (error) {
      Toast.show({
        type: 'error',
  ...
  const renderItem = ({ item }: { item: Banner }) => (
    <View style={styles.bannerItem}>
      <Image source={{ uri: `http://192.168.11.23/images/${item.image}` }} style={styles.bannerImage} />
