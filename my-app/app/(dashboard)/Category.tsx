...
const CategoryManagement = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get("http://192.168.11.23:8080/api/categories");
  ...
  const handleAddCategory = async () => {
    try {
      ...
      await axios.post("http://192.168.11.23:8080/api/categories", formData, {
  ...
  const handleEditCategory = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://192.168.11.23:8080/api/categories/${editingCategory.id}`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
  ...
  const handleDeleteCategory = async (id: number) => {
    try {
      await axios.delete(`http://192.168.11.23:8080/api/categories/${id}`);
      fetchCategories();
    } catch (error) {
      Toast.show({
        type: 'error',
