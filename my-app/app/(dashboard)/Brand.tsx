...
const BrandManagement = () => {
  ...
  const fetchBrands = async () => {
    try {
      const response = await axios.get("http://192.168.11.23:8080/api/brands");
  ...
  const handleAddBrand = async () => {
    try {
      ...
      await axios.post("http://192.168.11.23:8080/api/brands", formData, {
  ...
  const handleEditBrand = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://192.168.11.23:8080/api/brands/${editingBrand.id}`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
  ...
  const renderItem = ({ item }: { item: Brand }) => (
    <View style={styles.brandItem}>
      <Image
        source={{ uri: `http://192.168.11.23/images/${item.image}` }}
  ...
  const handleDeleteBrand = async (id: number) => {
    try {
      await axios.delete(`http://192.168.11.23:8080/api/brands/${id}`);
      fetchBrands();
    } catch (error) {
      Toast.show({
        type: 'error',
