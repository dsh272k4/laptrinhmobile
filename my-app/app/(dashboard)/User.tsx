...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://192.168.11.23:8080/api/users');
  ...
  const deleteUser = async (id: number) => {
    try {
      await axios.delete(`http://192.168.11.23:8080/api/users/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://192.168.11.23:8080/api/users/${selectedUser.id}`,
        form
      );
      setUsers(
