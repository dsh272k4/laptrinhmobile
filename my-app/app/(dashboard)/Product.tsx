...
import { View, Text, Button, FlatList, StyleSheet, Image, TouchableOpacity, Modal, TextInput, ScrollView, Switch, Alert } from 'react-native';
...
const ProductDashboard = () => {
  ...
  const onSubmit = async (data: IProductForm) => {
    if (updatedProduct) {
      ...
      try {
        const response = await axios.put(`http://192.168.11.23:8080/api/product/${updatedProduct.id}`, formData, {
          headers: {
  ...
  const getProducts = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://192.168.11.23:8080/api/productss');
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        "http://192.168.11.23:8080/api/categories"
      );
  ...
  const handleDeleteProduct = async (id: number) => {
    try {
      await axios.delete(`http://192.168.11.23:8080/api/product/${id}`);
      Toast.show({
        type: 'success',
        text1: 'Thành công',
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post("http://192.168.11.23:8080/api/upload-excel", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
  ...
  const toggleStatus = async (id: number, currentStatus: number) => {
    ...
    try {
      await axios.patch(`http://192.168.11.23:8080/api/${id}/status`, {
        status: newStatus,
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerContainert}>
        <TouchableOpacity onPress={() => router.push('/(dashboard)/ProductUpload')}>
      ...
      >
        <View style={styles.modalContainerr}>
          <View style={styles.modalContentt}>
            ...
            {file && <Text style={styles.fileName}>File đã chọn: {file.name}</Text>}
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 16,
  }, headerContainer: { flexDirection: 'row', alignItems: 'center', marginTop: 30 }, headerContainert: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
