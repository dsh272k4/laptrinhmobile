...
const OrderManagement = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get('http://192.168.11.23:8080/api/orders/excluding-status');
  ...
  const updateOrderStatus = async (orderId: number, status: string) => {
    try {
      ...
      const response = await axios.put(
        `http://192.168.11.23:8080/api/orders/${orderId}/status?status=${status}`,
  ...
  const renderItem = ({ item }: { item: Order }) => (
    <View style={styles.orderItem}>
      <View style={styles.textContainer}>
        <Text style={styles.orderText}>Mã đơn: {item.orderCode}</Text>
        ...
        <Text
          style={[
            ...
            item.status === 'CONFIRMED' && { color: '#28A745' }, // Màu cam
