...
const RegisterScreen = () => {
  ...
  const handleEmailBlur = async () => {
    if (!isValidEmail(email)) {
      ...
      try {
        const response = await axios.get(`http://192.168.11.23:8080/api/users/check-email`, {
          params: { email },
        });
  ...
  const handlePhoneBlur = async () => {
    if (!isValidPhone(phone)) {
      ...
      try {
        const response = await axios.get(`http://192.168.11.23:8080/api/users/check-phone`, {
          params: { phone },
        });
        if (response.status === 200) {
          Toast.show({
  ...
  const handleVerifyOtp = () => {
    if (otp === generatedOtp) {
      ...
      axios.post('http://192.168.11.23:8080/api/users/register', userData)
        .then(response => {
