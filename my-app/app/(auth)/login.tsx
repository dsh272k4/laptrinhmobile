...
export default function LoginScreen() {
  ...
  const handleEmailBlur = async () => {
    if (!isValidEmail(email)) {
      ...
      try {
        const response = await axios.get(`http://192.168.11.23:8080/api/users/check-email-login`, {
          params: { email },
        });
  ...
  const handlePhoneBlur = async () => {
    if (!isValidPhone(phone)) {
      ...
      try {
        const response = await axios.get(`http://192.168.11.23:8080/api/users/check-phone-login`, {
          params: { phone },
        });
        if (response.status === 200) {
          Toast.show({
  ...
  const handleLogin = () => {
    ...
    axios
      .post('http://192.168.11.23:8080/api/sms/send-otp', params, {
        headers: {
  ...
  const handleVerifyOtp = () => {
    if (otp === generatedOtp) {
      axios
        .get(`http://192.168.11.23:8080/api/users/login?email=${email}&phone=${phone}`)
        .then((response) => {
